"""Reference-driven backfill of gitee issues cited by OLK commits (ADR-022).

Scans kernel_commit.body for gitee.com/openeuler/kernel/issues/<ID> refs,
fetches each unique ID via /api/v5, upserts into bug table with
source='gitee'. ~4K unique IDs, ~66 min at 60 rpm anonymous.

Resumable: progress checkpointed under data/gitee_backfill_checkpoint.json.
"""
...
from ingest.gitee.fetcher import GiteeFetcher, GiteeServerError
from ingest.gitee.parser import parse_issue
from storage.pg.engine import get_engine

logger = logging.getLogger(__name__)

_ISSUE_ID_RE = re.compile(
    r"gitee\.com/openeuler/kernel/issues/([A-Z0-9]+)",
    re.IGNORECASE,
)
_CHECKPOINT = Path("data/gitee_backfill_checkpoint.json")
_SAVE_EVERY = 50


def extract_referenced_ids(engine) -> list[str]:
    """Distinct gitee issue IDs cited in kernel_commit.body."""
    sql = text(r"""
        SELECT DISTINCT (regexp_matches(
            body, 'gitee\.com/openeuler/kernel/issues/([A-Z0-9]+)', 'g'))[1] AS mid
          FROM kernel_commit
         WHERE body ILIKE '%gitee.com/openeuler/kernel/issues%'
    """)
    with engine.connect() as conn:
        rows = conn.execute(sql).fetchall()
    return sorted({r.mid.upper() for r in rows})
...
def _upsert_bug(conn, row: dict[str, Any]) -> None:
    """Upsert into bug table on (source, external_id)."""
    conn.execute(text("""
...
def backfill(*, limit: int | None = None, token: str | None = None) -> dict[str, Any]:
    """Fetch every cited gitee issue, upsert into bug. Skips already-fetched."""
    engine = get_engine()
    ckpt = _load_ckpt()
    ...
    if limit:
        todo = todo[:limit]
    logger.info("[gitee] %d total cited, %d already done (ok=%d 404=%d 5xx=%d), %d to fetch",
                len(all_ids), len(seen_done), len(fetched_set),
                len(not_found_set), len(server_err_set), len(todo))

    n_ok = n_404 = n_5xx = n_err = 0
    def _flush():
        ckpt["fetched"] = sorted(fetched_set)
        ckpt["not_found"] = sorted(not_found_set)
        ckpt["server_error"] = sorted(server_err_set)
        ckpt["last_run"] = datetime.now(timezone.utc).isoformat()
        _save_ckpt(ckpt)

    with GiteeFetcher(token=token) as fetcher:
        for i, iid in enumerate(todo, 1):
            try:
                raw = fetcher.get_issue(iid)
            except GiteeServerError as exc:
                # Deterministic 5xx — record and skip; do not waste future
                # resume cycles on it.
                logger.warning("[gitee] %s server error (skipped): %s", iid, exc)
                ...
                logger.error("[gitee] %s transient failure: %s", iid, exc)
                n_err += 1
                continue
            if raw is None:
                ...
                try:
                    ...
                    logger.error("[gitee] %s upsert failed: %s", iid, exc)
                    n_err += 1
                    continue
            if i % _SAVE_EVERY == 0:
                _flush()
                logger.info("[gitee] progress %d/%d  ok=%d 404=%d 5xx=%d err=%d",
